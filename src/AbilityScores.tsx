...
export function AbilityScores({state, dispatch}: {state: AbilityBlock, dispatch: Dispatch<ReducerAction>}) {
  ...
  const handleChange = (ability: keyof Abilities, valueKey: 'base' | 'enh' | 'size' | 'misc' | 'damage' | 'drain', value: string) => {
    dispatch({type: 'changeAbilities', payload: {ability, field: valueKey, value}})
  }
          ...
          {ABILITY_TYPES.map((ability) => (
            <tr key={ability}>
              ...
              {state.toggleDetail &&
                <>
                <td>
                <input type='number' value={state.abilities[ability].base}
                  onChange={(e) => handleChange(ability, 'base', e.target.value)}/>
                </td>
                <td>
                  <input type='number' value={state.abilities[ability].enh}
                    onChange={(e) => handleChange(ability, 'enh', e.target.value)}/>
                </td>
                <td>
                  <input type='number' value={state.abilities[ability].size}
                    onChange={(e) => handleChange(ability, 'size', e.target.value)}/>
                </td>
                <td>
                  <input type='number' value={state.abilities[ability].misc}
                    onChange={(e) => handleChange(ability, 'misc', e.target.value)}/>
                </td>
                <td>
                  <input type='number' value={state.abilities[ability].damage}
                    onChange={(e) => handleChange(ability, 'damage', e.target.value)}/>
                </td>
                <td>
                  <input type='number' value={state.abilities[ability].drain}
